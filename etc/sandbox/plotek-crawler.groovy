...
def address = "http://www.plotek.pl/plotek/1,78649,18799990,ewelina-lisowska.html"
def html = address.toURL().text
Document doc = Jsoup.parse(html)

println "Title: " + doc.select('.holder_top h1')?.first()?.text()
println "Date: " + Date.parse('dd.MM.yyyy HH:mm', doc.select('#gazeta_article_date')?.first()?.text())
println "Body: " + doc.select('div.cmsArtykulElem')?.first()?.text()?.substring(0, 100)
